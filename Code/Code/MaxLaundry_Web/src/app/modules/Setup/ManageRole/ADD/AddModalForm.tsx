...
import { GetTblAccesses, PostTblRole, PostTblRoleDetail } from '../../../../services/services'
        ...
        onSubmit: async (values, { setSubmitting }) => {
            ...
            try {
                if (isNotEmpty(values.roleDesc)) {
                    ...
                    await PostTblRole(newrole)
                        .then(async (response) => {
                            if (response.status == 200 || 201) {
                                SuccessToast({ message: 'New Role Added' })
                                await values.accessArray.forEach(async (accessId) => {
                                    await PostTblRoleDetail({ roleId: response.data.roleId, accessId: accessId, })
                                        .then((response) => { })
                                        .catch((reason) => { throw (reason) })
                                })
                                SuccessToast({ message: 'Access Assigned to Role' })
                                props.closeModal()
                            }
                        })
                        .catch((reason) => { throw (reason) })
